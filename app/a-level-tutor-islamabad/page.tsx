...
export default function ALevelTutorIslamabadPage() {
  return (
    <SubjectLandingPage
      ...
      subjectEmoji="📘"
      subjectOverview={[
        `A Level tutoring requests in Islamabad usually arrive in one of two very different forms: a student whose AS results came in lower than expected and now needs to recover before A2 content builds further on shaky ground, or a student who's managing fine academically but is simultaneously juggling <a href="/a-level-tutor-islamabad" class="text-[#C43D24] hover:underline font-black">A Level</a> revision with UCAS applications, predicted grades, and personal statements. Our tutors approach these two situations very differently.`,
        `For the first group, the priority is diagnosing exactly which AS topics are weak before A2 teaching assumes fluency with them, since Cambridge A Level questions regularly build A2 content directly on top of AS foundations. For the second group, the priority is often calendar management as much as content, mapping mock exam dates, UCAS deadlines and actual A2 exams onto one schedule so nothing collides in the same week.`,
        `Because most Islamabad A Level students are taking exactly 3 subjects (occasionally 4 for competitive programmes like medicine), we typically work with one tutor per subject rather than trying to find a single generalist across Physics, Economics and English Literature, say. Subject depth matters more at this level than at O Level, where broader familiarity across a subject is often enough.`,
      ]}
      subjectDetails={[
        `A first session with a returning AS student includes a quick review of their AS mock and exam results, identifying which specific topics (not just which subject) need rebuilding before new A2 content makes the gap harder to close.`,
        `For students juggling university applications, we help build a term-by-term calendar early in the AS year that maps UCAS deadlines (typically January for most UK courses) against school mock exams and actual A2 sittings, so revision time isn't accidentally squeezed by a collision that could have been planned around.`,
        `Across subjects, we push students toward topical past-paper practice well before full timed papers, since A Level questions frequently combine two or three topics in a single question, a pattern that only becomes visible once a student has seen enough varied questions on the same topic.`,
        `For predicted grades specifically, since these are usually based on AS results and A2 mock performance, we treat mock exams with the same seriousness as final exams, since a strong mock genuinely influences what grade a university sees on an application.`,
      ]}
      examBoards={['CAIE (Cambridge International)', 'Edexcel', 'Oxford AQA']}
      targetExams={['AS Level', 'A2 Level', 'Cambridge International A Level']}
      commonTopics={[
      ...
      whyChoose={[
        'For students recovering from a weak AS result, we diagnose specific weak topics before A2 content builds further on that same ground, rather than reteaching everything from scratch.',
        'For students managing university applications alongside A Level, we help build a term-by-term calendar mapping UCAS deadlines against mock exams and actual sittings.',
        'Tutors are assigned one subject each rather than a single generalist across a student\'s full combination, since subject depth matters more at A Level than at O Level.',
        'Mock exams are treated with the same seriousness as final exams, since predicted grades used in university applications are typically based on AS results and A2 mock performance.',
      ]}
      faqs={[
        {
          q: 'What qualifications do your A Level tutors in Islamabad have?',
