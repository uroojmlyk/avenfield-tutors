...
const inp = `w-full bg-white border-2 border-[#D4D0C5] rounded-xl px-3.5 py-2.5 text-[#2E4F5E] 
  placeholder:text-[#b0c4cc] text-sm font-semibold focus:outline-none focus:border-[#E8C86A] 
  focus:ring-2 focus:ring-[#E8C86A]/30 transition-all`
...
function PasswordModal({ onClose }: { onClose: () => void }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-[#2E4F5E]/70 backdrop-blur-sm">
      <div className="bg-[#FFFDF7] rounded-2xl w-full max-w-md border-2 border-[#2E4F5E] shadow-[8px_8px_0_0_#2E4F5E]">
        <div className="flex items-center justify-between px-6 py-5 border-b-2 border-[#E8C86A] bg-[#2E4F5E] rounded-t-2xl">
          <div className="flex items-center gap-2.5">
            <div className="w-8 h-8 bg-[#E8C86A] rounded-xl border-2 border-[#E8C86A]/50 flex items-center justify-center">
              <IconLock cls="w-4 h-4 text-[#2E4F5E]" />
            </div>
            <div>
              <p className="text-white font-black text-[15px] leading-tight">Change Password</p>
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4">
          ...
          <div className="flex gap-3 pt-1">
            <button type="button" onClick={onClose} className="flex-1 py-2.5 border-2 border-[#D4D0C5] text-[#2E4F5E] font-black hover:bg-[#E8C86A]/20 rounded-xl text-sm transition-all">Cancel</button>
            <button type="submit" disabled={loading}
              className="flex-1 py-2.5 bg-[#E05C42] text-white font-black rounded-xl border-2 border-[#a83c2a] shadow-[0_3px_0_0_#a83c2a] hover:shadow-[0_1px_0_0_#a83c2a] hover:translate-y-[2px] disabled:opacity-50 transition-all text-sm">
              {loading ? 'Updating…' : 'Update Password'}
            </button>
          </div>
        </form>
...
function EditModal({ tutor, onClose, onSave }: {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-[#2E4F5E]/70 backdrop-blur-sm">
      <div className="bg-[#FFFDF7] rounded-2xl w-full max-w-2xl max-h-[92vh] overflow-y-auto border-2 border-[#2E4F5E] shadow-[8px_8px_0_0_#2E4F5E]">

        {/* Header */}
        <div className="flex items-center justify-between px-6 py-5 border-b-2 border-[#E8C86A] sticky top-0 bg-[#2E4F5E] rounded-t-2xl z-10">
          <div className="flex items-center gap-2.5">
            <div className="w-8 h-8 bg-[#E8C86A] rounded-xl border-2 border-[#E8C86A]/50 flex items-center justify-center">
              <IconEdit cls="w-4 h-4 text-[#2E4F5E]" />
            </div>
            <div>
              <p className="text-white font-black text-[15px] leading-tight">Edit Tutor</p>
        ...
        <div className="p-6 space-y-5">
          ...
          {/* Photo */}
          <div className="flex items-center gap-5 p-4 bg-white rounded-2xl border-2 border-[#D4D0C5]">
            <div className="w-16 h-16 rounded-xl overflow-hidden border-2 border-[#2E4F5E] shadow-[3px_3px_0_0_#2E4F5E] flex-shrink-0">
              {preview
                ? <img src={preview} alt="preview" className="w-full h-full object-cover"/>
            ...
            <div className="flex-1">
              ...
              <div className="flex gap-2 flex-wrap">
                <label htmlFor="img-upload"
                  className={`inline-flex items-center gap-1.5 px-3 py-1.5 rounded-xl text-xs font-black cursor-pointer border-2 transition-all ${uploading ? 'opacity-50 cursor-not-allowed border-[#D4D0C5] text-[#b0c4cc] bg-white' : 'bg-[#E8C86A] text-[#2E4F5E] border-[#2E4F5E] shadow-[2px_2px_0_0_#2E4F5E] hover:shadow-none hover:translate-y-[1px]'}`}>
                  <IconUpload cls="w-3.5 h-3.5" />
                  {uploading ? 'Uploading…' : 'Upload Photo'}
                </label>
                {preview && (
                  <button type="button" onClick={() => { setForm(f => ({ ...f, imageUrl: '' })); setPreview('') }}
                    className="px-3 py-1.5 rounded-xl text-xs font-black bg-white text-[#E05C42] border-2 border-[#E05C42] hover:bg-[#E05C42] hover:text-white transition-all">
                    Remove
                  </button>
                )}
              </div>
          ...
          <div>
            <label className={lbl}>Subjects</label>
            <div className="flex flex-wrap gap-1.5 p-3 bg-white border-2 border-[#D4D0C5] rounded-xl max-h-32 overflow-y-auto">
              {SUBJECTS.map(s => (
                <button key={s} type="button"
                  onClick={() => setForm(f => ({ ...f, subjects: f.subjects?.includes(s) ? f.subjects.filter(x => x !== s) : [...(f.subjects||[]), s] }))}
                  className={`px-2.5 py-1 rounded-full text-[11px] font-black border-2 transition-all ${form.subjects?.includes(s) ? 'bg-[#2E4F5E] text-white border-[#2E4F5E]' : 'bg-white text-[#4a6a78] border-[#D4D0C5] hover:border-[#E8C86A] hover:text-[#2E4F5E]'}`}>
                  {s}
                </button>
              ))}
            </div>
          ...
          <div className="grid sm:grid-cols-2 gap-4">
            <div><label className={lbl}>Experience (years)</label><input type="number" className={inp} value={form.experience || 0} onChange={e => setForm(f => ({ ...f, experience: parseInt(e.target.value)||0 }))}/></div>
            <div>
              <label className={lbl}>Teaching Mode</label>
              <select className={inp} value={form.mode || 'both'} onChange={e => setForm(f => ({ ...f, mode: e.target.value as ITutor['mode'] }))}>
                <option value="both">Both — Online & Home Visit</option>
                <option value="online">Online Only</option>
                <option value="onsite">Home Visit Only</option>
          ...
          <div className="flex gap-3 pt-2 border-t-2 border-dashed border-[#E8C86A]">
            <button onClick={onClose} className="flex-1 py-2.5 border-2 border-[#D4D0C5] text-[#2E4F5E] font-black hover:bg-[#E8C86A]/20 rounded-xl text-sm transition-all mt-3">Cancel</button>
            <button onClick={handleSave} disabled={saving || uploading}
              className="flex-1 py-2.5 bg-[#3A9E8F] hover:bg-[#2d7a6e] text-white font-black rounded-xl border-2 border-[#2E4F5E] shadow-[3px_3px_0_0_#2E4F5E] hover:shadow-[1px_1px_0_0_#2E4F5E] hover:translate-y-[2px] disabled:opacity-50 transition-all text-sm mt-3">
              {saving ? 'Saving…' : 'Save Changes'}
            </button>
          </div>
        </div>
  ...
  return (
    <tr className="group border-b-2 border-[#E8C86A]/20 hover:bg-[#FFFDF7] transition-colors">
      ...
      <td className="px-4 py-4 hidden sm:table-cell">
        <span className={`inline-flex text-[10px] font-black px-2.5 py-1 rounded-full border-2 w-fit ${mode.bg} ${mode.text} ${mode.border}`}>
          {mode.label}
        </span>
        <p className="text-[#4a6a78] text-[11px] font-semibold mt-1">{t.experience} yr{t.experience !== 1 ? 's' : ''} exp</p>
      </td>

      {/* Contact */}
      <td className="px-4 py-4 hidden xl:table-cell">
        <a href={`https://wa.me/${t.whatsapp?.replace(/\D/g,'')}`} target="_blank" rel="noopener noreferrer"
          className="flex items-center gap-1.5 text-[#3A9E8F] hover:text-[#2d7a6e] text-[12px] font-black transition-colors group/wa">
          <IconWA cls="w-3.5 h-3.5 flex-shrink-0" />
          <span className="group-hover/wa:underline">{t.whatsapp}</span>
        </a>
      </td>
      ...
      <td className="px-5 py-4">
        <div className="flex items-center justify-end gap-1.5">
          ...
          <button onClick={onEdit}
            className="p-2 text-[#2E4F5E] bg-[#E8C86A]/30 hover:bg-[#E8C86A] rounded-lg border-2 border-[#E8C86A] transition-all" title="Edit">
            <IconEdit cls="w-3.5 h-3.5" />
          </button>
          <button onClick={onDelete}
            className="p-2 text-[#E05C42] bg-[#E05C42]/10 hover:bg-[#E05C42] hover:text-white rounded-lg border-2 border-[#E05C42] transition-all" title="Delete">
...
export default function AdminDashboard() {
  ...
  const navItems = [
    { key: 'pending'  as Filter, label: 'Pending Review', Icon: IconClock,  accent: '#E8C86A', accentText: 'text-[#2E4F5E]', activeBg: 'bg-[#E8C86A]' },
    { key: 'approved' as Filter, label: 'Approved',       Icon: IconCheck,  accent: '#3A9E8F', accentText: 'text-white',      activeBg: 'bg-[#3A9E8F]' },
    { key: 'rejected' as Filter, label: 'Rejected',       Icon: IconX,      accent: '#E05C42', accentText: 'text-white',      activeBg: 'bg-[#E05C42]' },
  ]

  const filterCfg = {
    pending:  { label: 'Pending',  dot: 'bg-[#E8C86A]', badge: 'bg-[#E8C86A]/25 border-[#E8C86A] text-[#2E4F5E]' },
    approved: { label: 'Approved', dot: 'bg-[#3A9E8F]', badge: 'bg-[#3A9E8F]/15 border-[#3A9E8F] text-[#2E4F5E]' },
    rejected: { label: 'Rejected', dot: 'bg-[#E05C42]', badge: 'bg-[#E05C42]/15 border-[#E05C42] text-[#E05C42]' },
  }

  return (
    <div className="min-h-screen flex bg-[#FFFDF7]" style={{ fontFamily: "'Nunito', sans-serif" }}>

      {/* ════════ SIDEBAR ════════ */}
      <aside className="w-[230px] flex-shrink-0 bg-[#2E4F5E] min-h-screen flex flex-col sticky top-0 h-screen border-r-4 border-[#E8C86A]">

        {/* Logo */}
        <div className="px-5 py-5 border-b-2 border-white/10">
          <div className="flex items-center gap-2.5">
            <div className="w-9 h-9 rounded-xl bg-[#E8C86A] border-2 border-[#E8C86A]/50 shadow-[2px_2px_0_0_rgba(0,0,0,0.2)] flex items-center justify-center flex-shrink-0">
            ...
            {navItems.map(({ key, activeBg, accentText }) => (
              <button key={key} onClick={() => setFilter(key)}
                className={`flex flex-col items-center p-2 rounded-xl border-2 transition-all ${
                  filter === key
                    ? `${activeBg} border-white/40 shadow-[2px_2px_0_0_rgba(0,0,0,0.2)]`
        ...
        <nav className="flex-1 px-3 py-4">
          ...
          {navItems.map(({ key, label, Icon, accent, accentText, activeBg }) => (
            <button key={key} onClick={() => setFilter(key)}
              className={`w-full flex items-center justify-between gap-2 px-3 py-2.5 rounded-xl mb-1 transition-all text-left border-2 ${
                filter === key
                  ? `${activeBg} border-white/30 shadow-[2px_2px_0_0_rgba(0,0,0,0.15)]`
                  : 'hover:bg-white/8 border-transparent'
              }`}>
              <div className="flex items-center gap-2.5">
                <span className={`${filter === key ? accentText : 'text-white/50'} transition-colors`}>
                  <Icon cls="w-4 h-4" />
                </span>
                <span className={`text-[13px] font-black transition-colors ${filter === key ? accentText : 'text-white/70 hover:text-white'}`}>{label}</span>
              </div>
              <span className={`min-w-[22px] text-center text-[10px] font-black px-1.5 py-0.5 rounded-full border-2 ${
                filter === key
                  ? `${accentText} bg-white/25 border-white/30`
        ...
        <div className="px-3 pb-5 border-t-2 border-white/10 pt-4 space-y-0.5">
          ...
          <a href="/" target="_blank" rel="noopener noreferrer"
            className="flex items-center gap-2.5 px-3 py-2.5 text-white/60 hover:text-white hover:bg-white/8 rounded-xl text-[13px] font-bold transition-all border-2 border-transparent">
            <IconExtLink cls="w-4 h-4" /> View Site
          </a>
          <button onClick={() => setShowPass(true)}
            className="w-full flex items-center gap-2.5 px-3 py-2.5 text-white/60 hover:text-[#E8C86A] hover:bg-[#E8C86A]/10 rounded-xl text-[13px] font-bold transition-all text-left border-2 border-transparent">
            <IconLock cls="w-4 h-4" /> Change Password
          </button>
          <button onClick={logout}
            className="w-full flex items-center gap-2.5 px-3 py-2.5 text-white/60 hover:text-[#E05C42] hover:bg-[#E05C42]/15 rounded-xl text-[13px] font-bold transition-all text-left border-2 border-transparent">
      ...
      <main className="flex-1 min-w-0 flex flex-col">
        ...
        {/* Topbar */}
        <header className="bg-[#FFFDF7] border-b-2 border-[#E8C86A] px-6 py-4 flex items-center justify-between sticky top-0 z-30">
          <div className="flex items-center gap-3">
          ...
          <span className={`inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full text-[11px] font-black border-2 ${filterCfg[filter].badge}`}>
            ...
            {loading ? '...' : tutors.length} {filterCfg[filter].label}
          </span>
        </header>
        ...
        <div className="flex-1 p-6">
          {loading ? (
            ...
            <div className="flex flex-col items-center justify-center py-40 bg-white rounded-2xl border-2 border-[#D4D0C5] shadow-[4px_4px_0_0_#D4D0C5]">
              <div className="w-16 h-16 bg-[#FFFDF7] rounded-2xl border-2 border-[#D4D0C5] flex items-center justify-center mb-4 text-3xl shadow-[2px_2px_0_0_#D4D0C5]">
                📭
              </div>
            ...
            <div className="bg-white rounded-2xl border-2 border-[#2E4F5E] shadow-[4px_4px_0_0_#2E4F5E] overflow-hidden">
                ...
                <table className="w-full">
                  <thead>
                    <tr className="border-b-2 border-[#E8C86A]/30 bg-[#FFFDF7]">
                      ...
                      <th className="px-4 py-3 text-left text-[10px] font-black text-[#2E4F5E] uppercase tracking-[0.18em] hidden sm:table-cell">Mode</th>
