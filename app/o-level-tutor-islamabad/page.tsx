...
export default function OLevelTutorIslamabadPage() {
  return (
    <SubjectLandingPage
      ...
      subjectEmoji="📖"
      subjectOverview={[
        `O Level tutoring is different from single-subject tutoring in one important way: most Islamabad families aren't looking for help in just one subject, they're managing 7-9 subjects at once and trying to work out where to actually spend limited time and budget. Our <a href="/tutors?subject=O%20Level&city=Islamabad" class="text-[#C43D24] hover:underline font-black">O Level tutors in Islamabad</a> often start not with a lesson, but with a conversation about which 2-3 subjects are genuinely at risk versus which ones the student is already managing fine without support.`,
        `That triage matters because O Level grades are cumulative across every subject taken, so a weak grade in one subject quietly drags down an otherwise strong result. We see this most often with <a href="/math-tutor-islamabad" class="text-[#C43D24] hover:underline font-black">Mathematics</a> and the sciences, since Islamabad's CAIE and Edexcel schools (Beaconhouse, Roots, Froebel's) move through content faster than local-board equivalents, and a student who falls slightly behind in class rarely catches up without outside help.`,
        `Because the Cambridge syllabus rewards exam technique as much as content knowledge (understanding what a command word like "describe" versus "explain" actually asks for), we build past-paper practice into every subject from early on rather than saving it for the final term, regardless of which specific subjects a student is taking.`,
      ]}
      subjectDetails={[
        `The first session with a new O Level family is usually a short audit: which subjects, which exam board (CAIE or Edexcel), current predicted grades if available, and which subjects the student or parent is genuinely worried about. This determines whether a student needs a single specialist tutor or a small coordinated team across two or three subjects.`,
        `For students taking multiple science subjects, we watch for a specific pattern: strong performance in class tests but weaker results on past papers, which usually signals a gap in exam technique rather than content knowledge. Fixing this is often faster than parents expect once it's correctly diagnosed.`,
        `Subject combination choices made now also affect <a href="/a-level-tutor-islamabad" class="text-[#C43D24] hover:underline font-black">A Level</a> options later, so for families still finalising subjects, our tutors can advise on which combinations keep the most doors open for common career paths like medicine, engineering, or business.`,
        `Across all subjects, the same core habits get built early: working from the actual syllabus document rather than assuming the textbook covers only what's examinable, and treating past papers as the main revision tool from at least two terms before exams, not just the final weeks.`,
      ]}
      examBoards={['CAIE (Cambridge International)', 'Edexcel', 'Oxford AQA']}
      targetExams={['O Level', 'IGCSE', 'Cambridge International']}
      commonTopics={[
      ...
      whyChoose={[
        'We start with a subject-by-subject audit to identify which 2-3 subjects are genuinely at risk, rather than assuming a student needs help across everything.',
        'Tutors are matched by exam board (CAIE or Edexcel) since mark schemes and question phrasing differ enough to matter, especially for exam technique.',
        'For students taking multiple sciences, we specifically watch for the class-test-strong-but-past-paper-weak pattern, which usually signals an exam-technique gap rather than a content gap.',
        'Subject combination advice accounts for A Level options later, so families choosing subjects now don\'t unintentionally close off common career paths.',
      ]}
      faqs={[
        {
          q: 'What qualifications do your O Level tutors in Islamabad have?',
