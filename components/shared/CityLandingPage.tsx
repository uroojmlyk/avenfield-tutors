    ...
    mainEntity: [
      {
        ...
        acceptedAnswer: {
          '@type': 'Answer',
          text: `Our verified tutors provide home tuition across ${Object.values(areas).flat().slice(0, 8).join(', ')} and many other areas in ${city}. If your specific area isn't listed, message us on WhatsApp and we'll confirm availability.`,
        },
      },
      {
        '@type': 'Question',
          ...
          <div className="space-y-3">
            {Object.entries(areas).map(([category, areaList]) => (
              <details
                ...
                className="group bg-white border-2 border-[#D4D0C5] rounded-xl overflow-hidden hover:border-[#3A9E8F] transition-colors"
              >
                <summary className="flex items-center justify-between px-5 py-3.5 cursor-pointer list-none hover:bg-[#FFFDF7] transition-colors">
                  <span className="text-[0.88rem] font-black text-[#2E4F5E]">
                    {getCategoryLabel(category)}
                    <span className="ml-2 text-[0.7rem] font-bold text-[#7da8b8] bg-[#D4D0C5]/30 px-2 py-0.5 rounded-full">
                      {areaList.length}
                    </span>
                  </span>
                  <span className="flex-shrink-0 w-6 h-6 rounded-full bg-[#E8C86A] flex items-center justify-center text-[#2E4F5E] font-black text-sm transition-transform group-open:rotate-180">
                ...
                <div className="px-5 pb-5 pt-1">
                  <div className="flex flex-wrap gap-2">
                    {areaList.map((area) => {
                      const areaURL = `/tutors?city=${encodeURIComponent(city)}&area=${encodeURIComponent(area)}`
                      return (
                        <Link
                          key={area}
                          href={areaURL}
                          className="px-3.5 py-2 bg-[#FFFDF7] text-[#2E4F5E] text-[0.78rem] font-bold rounded-full border-2 border-[#D4D0C5] hover:border-[#3A9E8F] hover:bg-[#E8C86A]/20 hover:shadow-[0_2px_8px_rgba(58,158,143,0.1)] transition-all no-underline"
                        >
                          {area}
                        </Link>
                      )
