  ...
  return (
    ...
    >
      <div className="max-w-6xl mx-auto px-4 sm:px-8 h-[72px] flex items-center justify-between gap-4">
        ...
        >
          {/* Icon box — border + shadow makes white-bg logo pop on cream navbar */}
          <div className="relative flex-shrink-0 w-[46px] h-[46px] sm:w-[52px] sm:h-[52px] rounded-xl border-2 border-[#2E4F5E] shadow-[2px_2px_0_0_#2E4F5E] overflow-hidden bg-white group-hover:shadow-[3px_3px_0_0_#E8C86A] group-hover:border-[#E8C86A] transition-all duration-200">
            <Image
              src="/avenfield-logo.jpeg"
              alt=""
              fill
              sizes="(max-width: 640px) 46px, 52px"
              className="object-contain object-center scale-[1.1]"
              priority
            />
          </div>

          {/* Brand text — always visible regardless of logo bg */}
          <div className="flex flex-col leading-tight">
            <span
              className="font-black tracking-[-0.01em] text-[1rem] sm:text-[1.15rem] text-[#2E4F5E] leading-none"
              style={{ fontFamily: "'Nunito', sans-serif" }}
            >
              Avenfield
            </span>
            <span
              className="font-black tracking-[0.12em] text-[0.62rem] sm:text-[0.7rem] text-[#E8C86A] uppercase leading-none mt-0.5"
              style={{ fontFamily: "'Nunito', sans-serif" }}
            >
              Tutors
            </span>
